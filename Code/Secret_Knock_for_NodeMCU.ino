/* Detects patterns of knocks and triggers a motor to unlock
   ...
   We have used a NodeMCU 1.0 ESP8266 as Microcontroller
 */
#include <ESP8266WiFi.h>
#include<Servo.h>

const char* ssid     = "Hi-Fi"; //Your Wifi Name
const char* password = "ScTiVCrMn"; //Your Wifi Password
...
const int programSwitch = D7;       // If this is high we program a new code.
const int lockMotor = D5;           // Gear motor used to turn the lock.
const int redLED = D2;              // Status LED
const int greenLED = D4;            // Status LED
const int output = D1;              // To Activate Secret Knocking Mechanism
...
const int lockTurnTime = 2000;      // milliseconds that we run the motor to get it to go a half turn.
    ...
    while (client.connected()) {            // loop while the client's connected
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          // if the current line is blank, you gost two newline characters in a row.
          ...
          if (currentLine.length() == 0) {
            ...
            if (header.indexOf("GET /1/on") >= 0) {
              Serial.println("Start Knocking Now");
              outputState = "on";
              digitalWrite(output, HIGH);
            } else if(header.indexOf("GET /1/off") >= 0) {
              Serial.println("Knocking mechanism is off");
              outputState = "off";
              digitalWrite(output, LOW);
            }
            ...
            // Web Page Heading
            client.println("<body><h1><center>Knock Knock</center></h1>");
...
void listenToSecretKnock(){
  ...
  do {
    ...
    if (knockSensorValue >=threshold){                   //got another knock...
      ...
      Serial.println(currentKnockNumber);
...
void triggerDoorUnlock(){
  ...
  for(servoAngle = 0; servoAngle < 180; servoAngle++)  //now move back the micro servo from 0 degrees to 180 degrees
  {                                
